...
//		root = ownLinkedList.insertStart(root, 9);
		...
		root = ownLinkedList.insertPosition(root, 1, 0);
		root = ownLinkedList.insertPosition(root, 4, 1);
		root = ownLinkedList.insertPosition(root, 9, 2);
		ownLinkedList.displayLinkedList(root);
		root = ownLinkedList.insertPosition(root, 3, 1);
//		root = ownLinkedList.insertPosition(root, 15, 10);  
		root = ownLinkedList.insertPosition(root, 15, 4);  
...
//		root.next.next = newNode;
	...
	public Node insertEnd(Node root, int val) {
		...
		/**  issue with blew code is that our root node is lost.
		 * Means root node became our last node but root node should be
		 * our starting node. 
